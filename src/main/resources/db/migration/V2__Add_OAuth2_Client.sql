...
(
    ...
    '{"@class":"java.util.Collections$UnmodifiableMap","settings.token.reuse-refresh-tokens":true,"settings.token.id-token-signature-algorithm":"RS256","settings.token.access-token-time-to-live":["java.time.Duration",3600],"settings.token.refresh-token-time-to-live":["java.time.Duration",86400]}' -- token_settings
)
...
(
    ...
    'client_secret_basic', -- client_authentication_methods
    'urn:ietf:params:oauth:grant-type:otp', -- authorization_grant_types (кастомный тип гранта для OTP в формате URN согласно RFC 6749)
    NULL, -- redirect_uris
    NULL, -- post_logout_redirect_uris
    'openid,profile,message.read', -- scopes
